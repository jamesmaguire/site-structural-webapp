...
{
    // Beam parameters
    input('i_B', {initval:1300, units:'mm'});
    input('i_D', {initval:800, units:'mm'});
    input('i_nbc', {initval:6});
    input('i_dbc', {initval:24, prefix:'N', align:'left'});
    input('i_nbt', {initval:6});
    input('i_dbt', {initval:24, prefix:'N', align:'left'});
    input('i_nbe', {initval:3});
    input('i_dbe', {initval:20, prefix:'N', align:'left'});
    input('i_dbs', {initval:16, prefix:'N', align:'left'});
    input('i_dbspc', {initval:150});
    input('i_c', {initval:45, units:'mm'});
    output('o_d', {units:'mm'});
    output('o_a', {units:'mm'});
    ...
    // Loading
    input('i_Mstar', {initval:840, units:'kNm'});
    input('i_Vstar', {initval:1600, units:'kN'});
...
function runCalcs() {
    ...
    concrete.alpha2 = Math.min(Math.max(0.67,1-0.003*concrete.fc, 0.67),0.85);
    concrete.gamma = Math.min(Math.max(0.67,1.05-0.007*concrete.fc), 0.85);
    ...
    o_Muo.value = Muo.toFixed(0);
    o_phi.value = phi.toFixed(2);
    o_phiMuo.value = (phi*Muo).toFixed(0);
    ...
    const kv = (0.4/(1+1500*epsilonx))*(1300/(1000+kdg*dv));
    ...
    // Tie check
    const yone = Math.max(beam.B,beam.D) - 2*beam.c - beam.dbs;
    const tiecheck = (0.2*yone/steel.fsyf) / (Asw*1000**2/s);
    o_tiecheck.value = tiecheck.toFixed(2);
    console.log(yone, tiecheck);
    setPassFail(o_tiecheck);
...
function bendingPhi(ku) {
    let phi = 1.19 - 13*ku/12;
    return Math.min(Math.max(phi, 0.6), 0.8);
}
