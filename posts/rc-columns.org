...
Steel area, A_{st} {{{output(o_Ast,mm<sup>2</mm>)}}}
...
Radius of gyration, r_x {{{output(o_rx,mm)}}}, r_y {{{output(o_ry,mm)}}}

Slenderness, \lambda{}_x {{{output(o_lambdax,)}}} \lambda{}_y {{{output(o_lambday,)}}} 

Radius of gyration, r {{{output(o_rd,mm)}}}

Slenderness, \lambda {{{output(o_lambdad,)}}}

Buckling load, N_{cx} {{{output(o_Ncx,kN)}}}

Squash load, N_{uo} {{{output(o_Nuo,kN)}}}

Mc = Mub with ku = 0.545 and phi = 0.65
